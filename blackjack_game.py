...
while run:
    ...
    if blackjack.initial_deal:
        for i in range(2):
            my_hand, game_deck = blackjack.deal_cards(my_hand, game_deck)
            dealer_hand, game_deck = blackjack.deal_cards(dealer_hand, game_deck)
        blackjack.initial_deal = False

    if active:
        player_score = blackjack.calculate_score(my_hand)
        blackjack.draw_cards(my_hand, dealer_hand, reveal_dealer)
        if reveal_dealer:
            dealer_score = blackjack.calculate_score(dealer_hand)
            if dealer_score < 17:
                dealer_hand, game_deck = blackjack.deal_cards(dealer_hand, game_deck)
        blackjack.draw_scores(player_score, dealer_score)
    buttons = blackjack.draw_game(active, records, outcome)
    ...
    outcome, records, add_score = blackjack.check_endgame(hand_active, dealer_score, player_score, outcome, records, add_score)
